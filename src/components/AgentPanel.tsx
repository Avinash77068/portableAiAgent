...
import { ArrowLeft, ChevronRight, FolderOpen, Loader2, Search, Send, Square, Wrench } from 'lucide-react'
...
export function AgentPanel({ repoRoot, autoApply, isRunning, steps, conversations, activeConversationId, onSelectFolder, onClearFolder, onSetAutoApply, onRun, onSelectConversation, onStartNewSession, onRenameConversation, onDeleteConversation, onApproveDiff, onRejectDiff, onStop, onClose }: AgentPanelProps) {
  ...
  const handleRename = (conversation: PortableAIAgentConversation) => {
    const nextTitle = window.prompt('Rename session', conversation.title)?.trim()
    if (nextTitle && nextTitle !== conversation.title) onRenameConversation(conversation.id, nextTitle)
  }

  const handleDelete = (conversation: PortableAIAgentConversation) => {
    if (window.confirm(`Delete "${conversation.title}"?\n\nThis action cannot be undone.`)) onDeleteConversation(conversation.id)
  }
        ...
        <div ref={transcriptRef} className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          {steps.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {steps.map(({ id, event }) => <StepView key={id} event={event} isRunning={isRunning} onApproveDiff={onApproveDiff} onRejectDiff={onRejectDiff} />)}
...
function StepView({ event, isRunning, onApproveDiff, onRejectDiff }: { event: AgentTranscriptEvent; isRunning: boolean; onApproveDiff: () => void; onRejectDiff: () => void }) {
  ...
  if (event.type === 'tool-call') {
    const detail = event.name === 'read_file' || event.name === 'write_file' ? String(event.args.path ?? '') : event.name === 'search_files' || event.name === 'web_search' ? String(event.args.query ?? '') : String(event.args.path ?? '.')
    return <div className="font-mono text-xs text-[var(--text-muted)]">→ {event.name}({detail})</div>
  }

  if (event.type === 'tool-result') return <ToolResultView result={event.result} />

  if (event.type === 'diff-pending') return <DiffView path={event.path} rows={event.rows} status="pending" onApprove={isRunning ? onApproveDiff : undefined} onReject={isRunning ? onRejectDiff : undefined} />
...
function ToolResultView({ result }: { result: string }) {
  ...
  return (
    <div className="rounded-lg bg-[var(--surface-soft)]">
      <button
        type="button"
        className="flex w-full items-center gap-1.5 px-2.5 py-1.5 text-left text-xs text-[var(--text-muted)] hover:text-[var(--text-secondary)]"
        onClick={() => setExpanded((value) => !value)}
      >
        <ChevronRight size={12} className={`shrink-0 transition-transform ${expanded ? 'rotate-90' : ''}`} />
        {expanded ? 'Hide result' : `Show result (${lineCount} line${lineCount === 1 ? '' : 's'})`}
      </button>
      {expanded && (
        <pre className="max-h-40 overflow-y-auto whitespace-pre-wrap px-2.5 pb-2.5 font-mono text-[0.72rem] text-[var(--text-secondary)]">{preview}</pre>
      )}
    </div>
  )
}
